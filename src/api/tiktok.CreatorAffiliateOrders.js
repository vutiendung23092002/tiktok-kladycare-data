...
export async function searchCreatorAffiliateOrders (next_page_token) {
    ...
    try {
        ...
        if (err.response) {
            writeSyncLog("ERROR", `[tiktok.CreatorAffiliateOrders.js] Status: ${err.response.status}`)
            writeSyncLog("ERROR", `[tiktok.CreatorAffiliateOrders.js] Data: ${JSON.stringify(err.response.data, null, 2)}`)
        } else {
            console.log(err.message);
            writeSyncLog("ERROR", `[tiktok.CreatorAffiliateOrders.js] Lỗi khi lấy danh sách đơn hàng: ${err.message}`)
        }
    }
}
